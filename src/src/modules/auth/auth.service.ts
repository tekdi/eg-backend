...
export class AuthService {
	...
	public async register(body, response) {
		...
		if (token?.access_token) {
			...
			if (registerUserRes.error) {
				...
				if (body.role === 'facilitator' || body.role === 'staff') {
	...
	public async sendSMS(mobileNo, message, args) {
		...
		let config = {
			...
			//url: `${process.env.SMS_GATEWAY_BASE_URL}/VoicenSMS/webresources/CreateSMSCampaignGet?ukey=${process.env.SMS_GATEWAY_API_KEY}&msisdn=${mobileNo},${args}&language=2&credittype=7&senderid=FEGGPR&templateid=0&message=${message}&isschd=false&isrefno=true&filetype=2`,
			url: `${process.env.SMS_GATEWAY_BASE_URL}/VoicenSMS/webresources/CreateSMSCampaignGet?ukey=${process.env.SMS_GATEWAY_API_KEY}&msisdn=${mobileNo}&language=2&credittype=7&senderid=FEGGPR&templateid=0&message=${message}&isschd=false&isrefno=true&filetype=2`,
			headers: {},
		};
